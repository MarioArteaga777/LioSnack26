...
const OrderCard = ({ pedido, onUpdate, onDetails, onDelete }) => {
  ...
  return (
    <div
      ref={containerRef}
      className={`relative w-full rounded-2xl bg-[#2A1F5E] p-5 shadow-xl overflow-hidden transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-2xl hover:shadow-black/40 hover:ring-1 hover:ring-sky-400/40 ${
    ...
    >
      ...
      <div className="relative space-y-3 text-white text-sm">
        <div className="flex justify-between gap-2">
          <span className="font-medium text-white/70">Producto</span>

          <span className="text-right">{resumenItems(pedido.items)}</span>
        </div>

        <div className="flex justify-between gap-2">
          <span className="font-medium text-white/70">Cliente</span>

          <span className="text-right">{pedido.cliente || "Sin cliente"}</span>
        </div>

        <div className="flex justify-between gap-2">
          <span className="font-medium text-white/70">Hora de pedido</span>
