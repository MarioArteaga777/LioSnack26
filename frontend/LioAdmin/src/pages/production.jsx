...
const Production = () => {
  const { productions, getProductions, loading } = useFetchProduction();

  const { createProduction, updateProduction, deleteProduction } =
  ...
  const handleSaveProduction = async (data) => {
    if (editingProduction) {
      const result = await updateProduction(editingProduction._id, data);
  ...
  if (loading) {
    return (
      <div className="flex h-screen items-center justify-center">
        <p className="text-white text-lg">Cargando Producción...</p>
      </div>
    );
  }
  ...
  return (
    <div>
      ...
      <div className="mb-20 flex items-center justify-between">
        ...
        <Button text="Nueva Producción" icon={Plus} onClick={openCreateForm} />
      </div>
          ...
          {productions.length > 0 ? (
            productions.map((production) => (
              <ProductionCard
                ...
                onUpdate={() => openUpdateForm(production)}
                onFinalize={() => setFinalizingProduction(production)}
                onDetails={() => setDetailsProduction(production)}
